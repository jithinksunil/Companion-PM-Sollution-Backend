...
const superUseController = {
    ...
    signUp: (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            if (superUserExist) {
                ...
                superUserSchema_1.default.insertMany([req.body]).then(() => {
                    console.log('added');
    ...
    logIn: (req, res) => {
        ...
        superUserSchema_1.default.findOne({ email }).then((superUserData) => {
            if (superUserData) {
                if (password === superUserData.password) {
                    ...
                    const token = jsonwebtoken_1.default.sign(superUser, 'mySecretKeyForSuperUser', { expiresIn: '1h' });
    ...
    superUserProfile: (req, res) => {
        superUserSchema_1.default.findOne({ _id: req.session.superUser._id }).then((superUserData) => {
    ...
    updateProfile: (req, res) => {
        ...
        if (req.session.superUser.password === password) {
            superUserSchema_1.default.findOneAndUpdate({ _id: req.session.superUser._id }, { $set: { name, email, companyName } }, { returnOriginal: false }).then((superUser) => {
    ...
    connections: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const projectManagers = yield projectManagerSchema_1.default.aggregate([{ $match: { superUserId } },
            {
                $lookup: {
    ...
    siteEngineerList: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        const superUserId = new mongoose_1.Types.ObjectId(req.session.superUser._id);
        const projects = yield projectSchema_1.default.aggregate([{ $match: { superUserId } }, {
                $lookup: {
        ...
        const unAssignedSiteEngineers = yield siteEngineerSchema_1.default.aggregate([{ $match: { superUserId } }, {
                $project: {
                    name: 1, projects: {
                        $filter: {
                            input: "$projects",
    ...
    siteEngineerAssignment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (startColumn !== endColumn) {
            if (startProject) {
                ...
                yield taskShema_1.default.updateMany({ projectId: startProject._id, "siteEngineers.siteEngineerId": new mongoose_1.Types.ObjectId(movingItemId) }, { $set: { "siteEngineers.$.status": false } });
    ...
    updateProjectAssingment: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            const superUserId = new mongoose_1.Types.ObjectId(req.session.superUser._id);
            const movingItemId = new mongoose_1.Types.ObjectId(movingItem._id);
            const endProjectManager = yield projectManagerSchema_1.default.findOne({ name: endColumn });
            const startProjectManager = yield projectManagerSchema_1.default.findOne({ name: startColumn });
            if (startProjectManager) {
                yield projectSchema_1.default.updateOne({ _id: movingItemId, "projectManagers.projectManagerId": startProjectManager._id }, { $set: { "projectManagers.$.status": false } });
            }
            let project;
            if (endProjectManager) {
                project = yield projectSchema_1.default.findOneAndUpdate({ _id: movingItemId, "projectManagers.projectManagerId": endProjectManager._id }, { $set: { "projectManagers.$.status": true } });
                if (!project) {
                    yield projectSchema_1.default.updateOne({ _id: movingItemId }, { $push: { projectManagers: { projectManagerId: endProjectManager._id, status: true } } });
                }
            }
    ...
    addConnection: (req, res) => {
        ...
        if (email && position) {
            ...
            (0, nodeMailer_1.mailService)(mailOptions).then(() => {
                if (position == 'projectManager') {
                    projectManagerSchema_1.default.insertMany([newConnectionData]).then(() => {
                    ...
                    siteEngineerSchema_1.default.insertMany([newConnectionData]).then(() => {
    ...
    paymentComplete: (req, res) => {
        superUserSchema_1.default.updateOne({ _id: req.session.superUser._id }, { $set: { membership: req.body.plan } }).then(() => {
