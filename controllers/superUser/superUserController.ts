...
import {
  createAndGetGuest,
  findGuest,
} from '../../dataBaserepository/guestRepository';
...
const superUseController = {
      ...
      .then((superUserData) => {
        if (superUserData) {
          if (password === superUserData.password) {
            ...
            const token = jwt.sign(superUser, 'mySecretKeyForSuperUser', {
              expiresIn: '1h',
            });
            res.json({
              verified: true,
  ...
  updateProfile: (req: reqType, res: resType) => {
    ...
    if (req.superUser.password === password) {
      superUserCollection
        .findOneAndUpdate(
          { _id: req.superUser._id },
          { $set: { name, email, companyName } },
  ...
  connections: async (req: reqType, res: resType) => {
    const superUserId = new Types.ObjectId(req.superUser._id);
    const projectManagers = await projectManagerCollection.aggregate([
      { $match: { superUserId } },
      {
        $lookup: {
  ...
  siteEngineerList: async (req: reqType, res: resType) => {
    const superUserId = new Types.ObjectId(req.superUser._id);
    const projects: any = await projectCollection.aggregate([
      { $match: { superUserId } },
      {
        $lookup: {
    ...
    const unAssignedSiteEngineers = await siteEngineerCollection.aggregate([
      { $match: { superUserId } },
      {
        $project: {
          name: 1,
  ...
  siteEngineerAssignment: async (req: reqType, res: resType) => {
    ...
    if (startColumn !== endColumn) {
      if (startProject) {
        await siteEngineerCollection.updateOne(
          { _id: movingItemId, 'projects.projectId': startProject._id },
          { $set: { 'projects.$.status': false, currentTaskOrder: [] } }
        );
        await taskCollection.updateMany(
          {
            projectId: startProject._id,
            'siteEngineers.siteEngineerId': new Types.ObjectId(movingItemId),
          },
      ...
      let siteEngineer;
      if (endProject) {
        siteEngineer = await siteEngineerCollection.findOneAndUpdate(
          { _id: movingItemId, 'projects.projectId': endProject._id },
          { $set: { 'projects.$.status': true } }
        );
        if (!siteEngineer) {
          await siteEngineerCollection.updateOne(
            { _id: movingItemId },
            { $push: { projects: { projectId: endProject._id, status: true } } }
          );
        }
      }
    }
  ...
  updateProjectAssingment: async (req: reqType, res: resType) => {
    try {
      ...
      const endProjectManager = await projectManagerCollection.findOne({
        name: endColumn,
      });
      const startProjectManager = await projectManagerCollection.findOne({
        name: startColumn,
      });
      ...
      if (endProjectManager) {
        project = await projectCollection.findOneAndUpdate(
          {
            _id: movingItemId,
      ...
      if (endColumn == 'unAssigned') {
        await siteEngineerCollection.updateMany(
          { superUserId, 'projects.projectId': movingItemId },
          { $set: { 'projects.$.status': false, currentTaskOrder: [] } }
        );
        await taskCollection.updateMany(
          { projectId: movingItemId, 'siteEngineers.status': true },
          { $set: { 'siteEngineers.$.status': false } }
        );
      }
      ...
      mailService(mailOptions)
        .then(() => {
          if (position == 'projectManager') {
            projectManagerCollection
              .insertMany([newConnectionData])
              .then(() => {
                res.redirect(
                  '/connections?message=Site engineer added successfully'
                );
              })
              .catch(() => {
                res.json({
                  status: false,
                  message:
                    'Connection cannot be added right now-database issue',
                });
              });
          } else if (position == 'siteEngineer') {
            siteEngineerCollection
              .insertMany([newConnectionData])
              .then(() => {
                res.json({
                  status: true,
