...
var projectManagerController = {
    ...
    signUp: function (req, res) {
        projectManagerSchema_1["default"].insertMany([req.body]).then(function () {
    ...
    logIn: function (req, res) {
        ...
        projectManagerSchema_1["default"].findOne({ logginUserName: logginUserName }).then(function (projectManager) {
            if (projectManager) {
                if (password === projectManager.password) {
                    ...
                    var projectManagerData = projectManager.toObject();
                    var token = jsonwebtoken_1["default"].sign(projectManagerData, 'mySecretKeyForProjectManager', { expiresIn: '1h' });
                    res.json({ verified: true, data: projectManager, message: 'Succesfully logged in', token: token });
                }
                else {
    ...
    projectManagerDashBoard: function (req, res) {
        var projectManagerData = req.session.projectManager;
        res.json({ tokenVerified: true, projectManagerData: projectManagerData });
    },
    projectManagerProfile: function (req, res) {
        projectManagerSchema_1["default"].findOne({ _id: req.session.projectManager._id }).then(function (projectManagerData) {
            var message = req.query.message;
            res.json({ tokenVerified: true, data: projectManagerData, status: true, message: message });
        })["catch"](function () {
            res.json({ tokenVerified: true, message: 'Cannot fetch data now data base issue' });
        });
    },
    updateImage: function (req, res) {
        var userId = req.session.projectManager._id;
        if (req.file) {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    ...
                    _a = req.session.projectManager, _id = _a._id, name_1 = _a.name;
    ...
    updateProfile: function (req, res) {
        ...
        if (req.session.projectManager.password === password) {
            projectManagerSchema_1["default"].findOneAndUpdate({ _id: req.session.projectManager._id }, { $set: { name: name, email: email, companyName: companyName } }, { returnOriginal: false }).then(function (projectManager) {
